...
Cloud Volumes ONTAP仅支持在AWS本地区域中进行HA部署。以下是示例配置：
...
Cloud Volumes ONTAP仅支持这些配置。在Cloud Volumes ONTAP的AWS本地区域配置中选择任何其他磁盘类型或EC2实例可能会导致部署失败。有关的最新完整详细信息，请参见AWS文档link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["本地区域中的EC2实例类型"^]。
