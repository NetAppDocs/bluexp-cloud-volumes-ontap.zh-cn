...
Cloud Volumes ONTAP 支持 https://["NetApp 卷加密（ NVE ）和 NetApp 聚合加密（ NAE ）"^]。NVE 和 NAE 是基于软件的解决方案，支持（ FIPS ） 140-2 合规的卷空闲数据加密。NVE 和 NAE 都使用 AES 256 位加密。
...
在 AWS 中启动 Cloud Volumes ONTAP 系统时，您可以使用启用数据加密 http://["AWS 密钥管理服务（ KMS ）"^]。BlueXP使用客户主密钥(CMK)请求数据密钥。
...
数据会使用在 Azure 中的 Cloud Volumes ONTAP 上自动加密 https://["Azure 存储服务加密"^] 使用 Microsoft 管理的密钥。
...
https://["Google Cloud Platform 空闲数据加密"^] 默认情况下， Cloud Volumes ONTAP 处于启用状态。无需设置。
