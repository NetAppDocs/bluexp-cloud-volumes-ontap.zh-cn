...
| AWS 服务的具体政府端点取决于您使用的 AWS 区域。端点后缀为 `amazonaws.com`和 `c2s.ic.gov`。参考	https://docs.aws.amazon.com/AWSJavaSDK/latest/javadoc/com/amazonaws/services/s3/model/Region.html["AWS SDK"^]和 https://docs.aws.amazon.com/general/latest/gr/rande.html["AWS 文档"^]了解更多信息。  a| 
* 云形成
* 弹性计算云（ EC2 ）
* 身份和访问管理(IAM)
* 密钥管理服务（ KMS ）
