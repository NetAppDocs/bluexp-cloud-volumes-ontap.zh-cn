...
TIP: 正在查找有关连接器的信息？ https://["查看Connector的安全组规则"^]



=== 数据分层连接

如果要将 EBS 用作性能层、将 AWS S3 用作容量层、则必须确保 Cloud Volumes ONTAP 与 S3 建立连接。提供该连接的最佳方法是创建到 S3 服务的 VPC 端点。有关说明，请参见 https://["AWS 文档：创建网关端点"^]。
...
要在AWS中的Cloud Volumes ONTAP 系统与其他网络中的ONTAP 系统之间复制数据、您必须在AWS VPC与其他网络(例如企业网络)之间建立VPN连接。有关说明，请参见 https://["AWS 文档：设置 AWS VPN 连接"^]。
