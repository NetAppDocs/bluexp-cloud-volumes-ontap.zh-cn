...
通过共享 VPC ，您可以跨多个项目配置和集中管理虚拟网络。您可以在 _host project_ 中设置共享 VPC 网络，并在 _service project_ 中部署 Connector 和 Cloud Volumes ONTAP 虚拟机实例。 https://["Google Cloud 文档：共享 VPC 概述"^]。

https://["查看 Connector 部署中涉及的所需共享 VPC 权限"^]



=== vPC 中的数据包镜像

https://["数据包镜像"^] 必须在部署 Cloud Volumes ONTAP 的 Google Cloud VPC 中禁用。如果启用了数据包镜像，则 Cloud Volumes ONTAP 无法正常运行。
...
如果您要将冷数据分层到 Google 云存储分段，则必须为 Cloud Volumes ONTAP 所在的子网配置专用 Google 访问（如果您使用的是 HA 对，则此子网位于 VPC-0 中）。有关说明，请参见 https://["Google Cloud 文档：配置私有 Google Access"^]。
...
有关说明，请参见 https://["Google Cloud 文档： Cloud VPN 概述"^]。
...
* VPC-1 ， VPC-2 和 VPC-3 中 HA 组件的另一组规则。这些规则适用于 HA 组件之间的入站和出站通信。 <<Rules for VPC-1,vPC-2和vPC-3、了解更多信息>>。



TIP: 正在查找有关连接器的信息？ https://["查看Connector的防火墙规则"^]
...
如果您使用自己的防火墙策略、请确保添加需要与Cloud Volumes ONTAP 通信的所有网络、但同时确保同时添加两个地址范围、以使内部Google负载平衡器正常运行。这些地址为 130.11.0.0/22 和 35.191.0.0/16 。有关详细信息，请参见 https://["Google Cloud 文档：负载平衡器防火墙规则"^]。
