...
内部负载平衡器:: BlueXP会自动创建四个Google Cloud内部负载平衡器(TCP/UDP)、用于管理传入到Cloud Volumes ONTAP HA对的流量。您无需进行任何设置我们将此列为一项要求，只是为了告知您网络流量并缓解任何安全问题。
+
--
...
--
HA 对的一个或多个分区:: 您可以通过在多个或单个分区中部署 HA 配置来确保数据的高可用性。在创建HA对时、BlueXP将提示您选择多个分区或单个分区。
+
--
* 多个分区（建议）
+
...
专用 IP 地址:: BlueXP在GCP中为Cloud Volumes ONTAP 分配以下数量的专用IP地址：
+
--
* * 单节点 * ： 3 或 4 个专用 IP 地址
+
...
* VPC-0 中 HA 组件的一组规则。这些规则允许对 Cloud Volumes ONTAP 进行数据访问。 <<Cloud Volumes ONTAP 的防火墙规则,了解更多信息。>>。
* VPC-1 ， VPC-2 和 VPC-3 中 HA 组件的另一组规则。这些规则适用于 HA 组件之间的入站和出站通信。 <<Cloud Volumes ONTAP 的防火墙规则,了解更多信息。>>。
...
BlueXP会创建包含Cloud Volumes ONTAP 成功运行所需的入站和出站规则的GCP防火墙规则。您可能需要参考端口进行测试，或者如果您希望使用自己的防火墙规则。
...
在 GCP 中， HA 配置部署在四个 VPC 上。VPC-0 中的 HA 配置所需的防火墙规则为 <<Cloud Volumes ONTAP 的防火墙规则,上面列出的 Cloud Volumes ONTAP>>。
