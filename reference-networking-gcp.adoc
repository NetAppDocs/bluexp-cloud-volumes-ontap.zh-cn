...
BlueXP会自动将所需数量的专用IP地址分配给GCP中的Cloud Volumes ONTAP。您需要确保网络具有足够的可用私有地址。

Cloud Manager 为 Cloud Volumes ONTAP 分配的 LIF 数量取决于您部署的是单节点系统还是 HA 对。LIF 是与物理端口关联的 IP 地址。SnapCenter 等管理工具需要 SVM 管理 LIF 。
...
* *高可用性对* BlueXP会将14个IP地址分配给一个高可用性对：
+
** 2个节点管理LIF (e0a)
** 2个iSCSI LIF (e0a)
+
...
TIP: 正在查找有关连接器的信息？ https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-ports-gcp.html["查看Connector的防火墙规则"^]
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-gcp.html["查看连接器的网络要求"^]
