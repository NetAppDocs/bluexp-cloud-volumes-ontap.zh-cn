...
summary: BlueXP会创建AWS安全组、其中包含BlueXP和Cloud Volumes ONTAP 成功运行所需的入站和出站规则。您可能需要参考端口进行测试，或者需要使用自己的端口。 
---
= AWS 的安全组规则
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
BlueXP会创建AWS安全组、其中包含Connector和Cloud Volumes ONTAP 成功运行所需的入站和出站规则。您可能希望参考这些端口进行测试或使用自己的安全组。
