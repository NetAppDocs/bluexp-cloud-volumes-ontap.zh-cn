...
* 指定新卷的详细信息、并让BlueXP为您处理底层数据聚合。  a volume,了解更多信息。。
* 在您选择的数据聚合上创建卷。  a volume,了解更多信息。。
* 使用模板创建卷，以根据特定应用程序（例如数据库或流式服务）的工作负载要求优化卷。  a volume from a template,了解更多信息。。
* 在 HA 配置中的第二个节点上创建卷。  a volume on the second node in an HA configuration,了解更多信息。。
...
* 创建iSCSI卷时、BlueXP会自动为您创建LUN。我们通过为每个卷仅创建一个 LUN 来简化此过程，因此无需进行管理。创建卷后，  a LUN to a host,使用 IQN 从主机连接到 LUN。
...
. 单击 image:screenshot_gallery_options.gif["由三个并排点组成的图标。"] > * 从模板添加卷 * 。
+
...
. 为模板中未硬编码的所有参数添加值。请参见  a volume,创建卷 有关部署 Cloud Volumes ONTAP 卷所需完成的所有参数的详细信息。
