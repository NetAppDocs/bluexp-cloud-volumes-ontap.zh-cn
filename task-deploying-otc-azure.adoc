...
+
从Cloud Volumes ONTAP 9.15.1开始、您可以在本地冗余存储(LRS)单可用性区域中部署VM实例。这也需要 `Microsoft.Compute/VMOrchestratorZonalMultiFD`为您的订阅激活此功能。此功能有助于在同一可用性区域中部署高可用性(HA)对。请参阅link:task-saz-feature.html["为单个可用性区域启用VMOrchestratorzonalMultiFD"]和link:task-azure-high-availability-mode.html["在Azure中启用高可用性模式"]。
...
+
*** 对于单个可用性区域、请选择Azure区域、可用性区域、vNet和子网。从Cloud Volumes ONTAP 9.15.1开始、您可以在单个可用性区域中安装VM实例。您需要选择支持此模式的区域。如果您选择的区域不支持分区部署、则会遵循以前的LRS非分区部署模式。
+
要了解有关此部署模式和支持的Azure区域的详细信息，请参阅 https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Microsoft Azure文档：虚拟机扩展集文档"^]和 https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Microsoft Azure网站：产品按地区提供"^]。
...
. * 使用情况配置文件，磁盘类型和分层策略 * ：选择是否要启用存储效率功能，并根据需要更改卷分层策略。
+
有关详细信息，请参阅 link:https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["选择卷使用情况配置文件"^] 和 link:concept-data-tiering.html["数据分层概述"^]。
