...
此连接器应部署在与其管理的 Cloud Volumes ONTAP 系统所在的同一 Azure 区域或中 https://["Azure 区域对"^] 对于 Cloud Volumes ONTAP 系统。
...
https://["了解如何在Connector上启用直接API流量"^]
...
如果您使用 https://["Azure 专用 DNS"^]，然后您需要修改每个 Connector 上的配置文件。否则、BlueXP无法在Cloud Volumes ONTAP 与其关联存储帐户之间启用Azure专用链路连接。

请注意， DNS 名称必须与 Azure DNS 命名要求匹配 https://["如 Azure 文档中所示"^]。
