...
summary: '与标准AWS区域类似、您可以在AWS商用云服务(C2S)环境中使用BlueXP (以前称为Cloud Manager)来部署Cloud Volumes ONTAP 、从而为您的云存储提供企业级功能。AWS C2S 是一个特定于美国的封闭区域智能社区；此页面上的说明仅适用于 AWS C2S 区域用户。' 
---
...
[role="lead"]
与标准AWS区域类似、您可以在中使用BlueXP (以前称为Cloud Manager) https://aws.amazon.com/federal/us-intelligence-community/["AWS 商用云服务（ C2S ）"] 部署 Cloud Volumes ONTAP 的环境，为您的云存储提供企业级功能。AWS C2S 是一个特定于美国的封闭区域智能社区；此页面上的说明仅适用于 AWS C2S 区域用户。



== C2S 中支持的功能

在C2S环境中、BlueXP提供了以下功能：
...
. 安装Connector并设置BlueXP。
+
在开始使用BlueXP部署Cloud Volumes ONTAP 之前、您需要先创建_Connector_。借助此连接器、BlueXP可以管理公有 云环境(包括Cloud Volumes ONTAP)中的资源和流程。

+
您将从安装在Connector实例上的软件登录到BlueXP。

. 从BlueXP启动Cloud Volumes ONTAP。
...
要从BlueXP部署Cloud Volumes ONTAP 、需要订阅市场。
...
+
您必须使用BlueXP启动Cloud Volumes ONTAP 实例。不得从 EC2 控制台启动 Cloud Volumes ONTAP 实例。
...
设置IAM策略和角色、为BlueXP和Cloud Volumes ONTAP 提供在AWS商用云服务环境中执行操作所需的权限。
...
+
在从BlueXP创建Cloud Volumes ONTAP 工作环境时、您可以为Cloud Volumes ONTAP 和HA调解器选择IAM角色。
...
在AWS中启动Cloud Volumes ONTAP 系统之前、您必须先从AWS Marketplace启动Connector实例、然后登录并设置BlueXP。
...
+
.. 打开 Web 浏览器并输入以下 URL ： http://ipaddress[]
.. 指定用于连接到 AWS 服务的代理服务器。
.. 上传您在步骤 1 中获得的证书。
.. 完成设置向导中的步骤以设置BlueXP。
+
*** *系统详细信息*：输入此BlueXP实例的名称并提供您的公司名称。
*** *创建用户*：创建用于管理BlueXP的管理员用户。
...
您可以通过在BlueXP中创建新的工作环境在AWS商用云服务环境中启动Cloud Volumes ONTAP 实例。
...
BlueXP创建了安全组、其中包括BlueXP和Cloud Volumes ONTAP 在云中成功运行所需的入站和出站规则。您可能需要参考端口进行测试，或者如果您希望使用自己的安全组。
...
为 Cloud Volumes ONTAP HA 调解器预定义的内部安全组包括以下规则。BlueXP始终会创建此安全组。您无法选择使用自己的。
