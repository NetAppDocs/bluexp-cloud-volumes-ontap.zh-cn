...
[role="lead"]
与标准 AWS 区域类似，您可以在中使用 Cloud Manager https://["AWS 商用云服务（ C2S ）"] 部署 Cloud Volumes ONTAP 的环境，为您的云存储提供企业级功能。AWS C2S 是一个特定于美国的封闭区域智能社区；此页面上的说明仅适用于 AWS C2S 区域用户。
