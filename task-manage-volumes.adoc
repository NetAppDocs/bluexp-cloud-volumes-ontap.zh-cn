...
如果需要调整卷大小，可以通过执行此操作 link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.onc-sm-help-960/GUID-C04C2C72-FF1F-4240-A22D-BE20BB74A116.html["ONTAP 系统管理器"^]。调整卷大小时，请务必考虑系统的容量限制。转至 https://["《 Cloud Volumes ONTAP 发行说明》"^] 有关详细信息：
...
+
image:screenshot_aggr_volume_overview.png["屏幕抓图：显示聚合中聚合中卷的列表（在 \" 聚合信息 \" 对话框中）。"]
...
+
有关详细信息，请参见 link:https://docs.netapp.com/us-en/cloud-manager-replication/task-replicating-data.html["在系统之间复制数据"^]。

. 转到复制状态页，然后中断 SnapMirror 关系、将复制的卷从数据保护卷转换为读 / 写卷。
+
有关详细信息，请参见 link:https://docs.netapp.com/us-en/cloud-manager-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["管理数据复制计划和关系"^]。
...
+
image:screenshot_aggr_capacity.png["屏幕抓图：显示聚合信息对话框中可用的总聚合容量和已用聚合容量。"]
...
image:screenshot_show_flexgroup_vol.png["显示FlexGroup 卷图标将文本悬停在卷磁贴下的屏幕截图。"]
