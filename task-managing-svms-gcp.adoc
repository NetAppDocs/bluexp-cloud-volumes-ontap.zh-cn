...
从9.11.1版开始、Google Cloud中的特定Cloud Volumes ONTAP 配置支持多个Storage VM。转至 https://["《 Cloud Volumes ONTAP 发行说明》"^] 验证您的 Cloud Volumes ONTAP 版本支持的 Storage VM 数量。
...
+
https://["Google Cloud文档：向现有实例添加别名IP范围"^]
...
.所需的Google Cloud权限
连接器需要特定的权限来创建和管理Cloud Volumes ONTAP HA对的Storage VM。所需权限包括在中 https://["NetApp 提供的策略"]。
