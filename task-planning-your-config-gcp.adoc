...
大多数 Google Cloud 地区均支持 Cloud Volumes ONTAP 。 https://["查看支持的区域的完整列表"^]。
...
https://["GCP 中支持的 Cloud Volumes ONTAP 配置"^]
...
https://["GCP 中 Cloud Volumes ONTAP 的存储限制"^]
...
计算机类型:: 在中查看支持的计算机类型 http://["《 Cloud Volumes ONTAP 发行说明》"^] 然后查看 Google 提供的有关每个受支持计算机类型的详细信息。将工作负载要求与此计算机类型的 vCPU 和内存数量相匹配。请注意，每个 CPU 核心都会提高网络连接性能。
+
--
有关更多详细信息，请参见以下内容：

* https://["Google Cloud 文档： N1 标准计算机类型"^]
* https://["Google Cloud 文档：性能"^]
...
+
有关详细信息，请参见 https://["Google Cloud 文档：区域持久性磁盘（标准和 SSD ）"^]。
...
+
** https://["Google Cloud 文档：区域持久性磁盘（标准和 SSD ）"^]
** https://["Google Cloud 文档：优化持久磁盘和本地 SSD 性能"^]
