...
[role="lead"]
数据会使用在 Azure 中的 Cloud Volumes ONTAP 上自动加密 https://["Azure 存储服务加密"] 使用 Microsoft 管理的密钥。但是，您可以按照此页面上的步骤使用自己的加密密钥。



== 数据加密概述

Cloud Volumes ONTAP 数据在 Azure 中使用自动加密 https://["Azure 存储服务加密"^]。默认实施使用 Microsoft 管理的密钥。无需设置。
...
* 从ONTAP 9.10.1P3开始、对于NVRAM和核心磁盘、BlueXP使用 https://["磁盘加密集"^]，用于管理具有受管磁盘的加密密钥。较低版本将使用Microsoft管理的密钥、而不是客户管理的密钥。
...
* 对于根磁盘、启动磁盘和数据磁盘、BlueXP使用 https://["磁盘加密集"^]，用于管理具有受管磁盘的加密密钥。
* 任何新数据磁盘也会使用相同的磁盘加密集。
