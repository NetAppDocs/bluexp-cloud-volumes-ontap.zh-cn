...
确保 Cloud Volumes ONTAP 已连接到 S3 。提供该连接的最佳方法是创建到 S3 服务的 VPC 端点。有关说明，请参见 https://["AWS 文档：创建网关端点"^]。
...
----
默认情况下、权限包括在自定义角色中。 https://["查看Connector的Azure权限"^]
...
* 必须为 Cloud Volumes ONTAP 所在的子网配置专用 Google 访问。有关说明，请参见 https://["Google Cloud 文档：配置私有 Google Access"^]。
...
. <<要求,确保满足所有要求>>。
. 从左侧导航菜单中、选择*存储>画布*。
. 在"画布"页面上、选择Cloud Volumes ONTAP 实例的名称。
. 单击菜单图标并选择 * 启用容量分层 * 。
+

NOTE: 只有在BlueXP创建系统时无法启用数据分层时、您才会看到此选项。

+
在Google Cloud中、必须先将服务帐户附加到Cloud Volumes ONTAP 、然后才会显示此选项。 <<要求,确保满足所有要求>>。
...
+
有关复制数据的帮助，请参见 https://["将数据复制到云中或从云中复制数据"^]。
