...
* 您无需在系统级别启用数据分层。
+
Cloud Manager 会在创建系统时为冷数据创建一个对象存储，  data tiering after implementing the requirements,只要没有连接或权限问题。之后，您只需在卷上启用数据分层（在某些情况下，  that tiering is enabled on aggregates,在聚合上）。
