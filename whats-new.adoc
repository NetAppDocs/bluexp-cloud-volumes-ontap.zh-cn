...
所有Cloud Volumes ONTAP许可证均附带WORM (一次写入、多次读取)和ARP (自动防兰软件保护)的内置数据保护和安全功能、无需额外付费。对于基于节点的许可证和基于容量的许可证、这适用于新的和现有的BYOL和PAYGO/市场部署。新的定价模式适用于所有云提供商和部署类型、包括单节点和高可用性(HA)配置。

将提供以下优势：

* 使用WORM和ARP的现有帐户将不再为这些功能付费。我们将根据此次更改之前适用的容量使用情况向您计费。WORm和ARP将在将来的计费中排除。
* 如果您的现有帐户未启用这些功能、您现在可以选择WORM和ARP、无需额外费用。
* 对于新客户、WORM和ARP费用将从所有Cloud Volumes ONTAP产品中消除。
