...
此页面上介绍的增强功能专用于支持Cloud Volumes ONTAP 管理的BlueXP功能。要了解 Cloud Volumes ONTAP 软件本身的新增功能， https://["转至《 Cloud Volumes ONTAP 发行说明》"^]
...
https://["了解此版本 Cloud Volumes ONTAP 中的新增功能"^]。
...
https://["了解如何使用NetApp加密解决方案对卷进行加密"^]。
...
https://["了解如何在 ONTAP 中配置和管理 S3 对象存储服务"^]
...
有关如何配置和使用NDMP的信息、请参见 https://["NDMP 配置概述"]。
...
有关此新功能的详细信息、请参见 https://["设置 Cloud Volumes ONTAP 以在 Azure 中使用客户管理的密钥"]。
...
* *边缘缓存*：许可 https://["Cloud Volumes Edge Cache"^]


https://["了解有关这些许可包的更多信息"]。
...
https://["查看Cloud Volumes ONTAP 支持的区域的完整列表"^]
...
"ec2:ModifyVolume",
----
请务必为您添加到BlueXP中的每组AWS凭据提供这些权限。 https://["查看AWS的最新Connector策略"^]。
...
https://["了解有关使用Cloud Volumes ONTAP 连接Azure专用链路的更多信息"]。
...
* https://["详细了解Google Cloud中的Storage VM限制"^]
* https://["了解如何在Google Cloud中为Cloud Volumes ONTAP 创建提供数据的Storage VM"]
...
https://["了解BlueXP中的Kubernetes数据管理"^]
...
https://["在Google Cloud中查看支持的Cloud Volumes ONTAP 配置"^]
...
您仍然可以通过在连接到 Cloud Volumes ONTAP 系统的 Web 浏览器中输入集群管理 IP 地址来连接到 System Manager 。 https://["了解有关连接到 System Manager 的更多信息"]。
...
https://["详细了解 Cloud Volumes ONTAP 的许可选项"]。
...
转至 https://["《 Cloud Volumes ONTAP 发行说明》"^] 有关支持的配置的更多详细信息。
...
* https://["了解有关NetApp Keystone 订阅的更多信息"^]。
...
数据会使用在 Azure 中的 Cloud Volumes ONTAP 上自动加密 https://["Azure 存储服务加密"^] 使用 Microsoft 管理的密钥。但是，您现在可以通过完成以下步骤来使用自己的客户管理的加密密钥：
...
https://["查看 AWS 中支持的 Cloud Volumes ONTAP 配置"^]。
...
----
请务必为您添加到BlueXP中的每组Azure凭据提供这些权限。 https://["查看Azure的最新Connector策略"^]。
...
https://["在 GCP 中查看支持的 Cloud Volumes ONTAP 配置"^]。
...
转至 https://["AWS Marketplace 页面"^] 要查看定价详细信息，请转到 https://["《 Cloud Volumes ONTAP 发行说明》"^] 了解有关此许可选项的更多信息。
...
请注意， Connector 需要新的权限才能删除 Azure 快照。 https://["查看Azure的最新Connector策略"^]。
...
现在，如果您在单节点系统中使用 E32S_v3 VM 类型，则 Cloud Volumes ONTAP 可以使用超 SSD 作为 VNVRAM https://["在任何受支持的 Azure 区域"^]。
